              ...
              return (
                <>
                  ...
                  {items.length === 0 ? <p className="text-white/30 text-sm text-center py-12">{likeFilter === 'works' ? '还没有喜欢任何作品' : '还没有喜欢任何帖子'}</p> :
                    isFeed ? (
                      <div className="divide-y divide-white/5">{items.map(p => (<Link key={p.id} href={`/post/${p.id}`} className="block p-4 hover:bg-white/[0.02] transition"><p className="text-sm mb-2">{p.caption}</p>{p.hashtags?.length>0 && <p className="text-[#f472b6] text-xs mb-2">{p.hashtags.map((h:string) => '#'+h).join(' ')}</p>}{p.media_url && (p.type==='video' ? <video src={p.media_url} className="w-full max-h-60 object-contain rounded-xl" controls preload="metadata" /> : <img src={p.media_url} className="w-full max-h-60 object-contain rounded-xl" alt="" />)}<div className="text-white/40 text-xs mt-2">{p.created_at ? new Date(p.created_at).toLocaleDateString() : ''}</div></Link>))}</div>
                    ) : (
